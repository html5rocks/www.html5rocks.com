...
EngineRect.prototype.update = function() {
  ...
  if (this.render & engine.RENDER_DOM) {
    if ((engine.debugLimitToPlane != -1) &&
        (engine.debugLimitToPlane.indexOf(this.actor.rect.plane) == -1)) {
      this.actor.rect.el.style.visibility = 'hidden';
    } else {
      this.actor.rect.el.style.visibility = 'visible';      
    }
  }
...
engine.updateRectOrder = function() {
  ...
  for (var i in engine.actors) {
    if (engine.actors[i].rect.visible) {
      ...
      if ((engine.debugLimitToPlane != -1) &&
          (engine.debugLimitToPlane.indexOf(engine.actors[i].rect.plane) == -1)) {
        continue;
      }
...
engine.debugSetLimitToPlane = function(params) {
  if (params && params.limitToPlane) {
    engine.debugLimitToPlane = params.limitToPlane;
  } else {
    engine.debugLimitToPlane = -1;
  } 
  ...
  'satellite': {
    ...
    startPassage: function() {
      ...
      this.addTransition({
          ...
          onFinish: function() {
            engine.addEvent({
              startTime: engine.rangeRand({ min: 0, max: 2000 }),
